...
\label{chap:llm_augmentation}
%\epigraph{\itshape  La volonté trouve, la liberté choisit. Trouver et choisir, c'est penser}{-- Victor Hugo}

\startcontents[chapters]
\printmyminitoc{
	%\lipsum[1]
\textit{Chapter summary.} RSVQA models trained on automatically generated, template-based datasets learn to exploit surface-level linguistic patterns rather than robust visual understanding, and degrade sharply when questions are reformulated. This chapter addresses this generalization gap from a data-centric perspective: we use an instruction-tuned LLM (LLaMA-2-7B) to generate ten syntactically diverse, semantically equivalent reformulations of each question in RSVQA-HR, guided by a prompt selected through systematic evaluation of nine candidates. Compared against back translation, the proposed augmentation improves robustness to unseen phrasings while preserving in-distribution accuracy, establishing question diversity as an important concern in RSVQA training data.
...
generated from geospatial vectorial databases such as OpenStreetMap~\cite{osm}\cite{lobry2020rsvqa},
...
unpredictably~\cite{chappuis2025langbias}.
...
paired with a text encoder such as BERT~\cite{sanh2019distilbert},
 followed by a fusion module, then classification for answer
prediction over a fixed set of predefined answers. This
paradigm does not itself forbid generative capacity, but in
practice does not rely on autoregressive text generation: the model has no world
...
robustness within the then-dominant classification-based RSVQA
framework.
...
generalization to unseen question formulations. Our results
show that LLM-based augmentation improves robustness to
unseen phrasings substantially over both the original,
non-augmented model and the back-translation baseline, while
preserving in-distribution accuracy, indicating that the
syntactic diversity of LLM-generated reformulations, rather
than back translation's more limited variation, is what
...
\label{sec:chap3_rw}
This section situates the proposed approach within three parts. Section~\ref{sec:chap3_rw_nlp} reviews data augmentation techniques developed in the general NLP literature, from rule-based token perturbation to back translation, which form a baseline this chapter compares against. Section~\ref{sec:chap3_rw_vqa} narrows this to VQA and RSVQA specifically, where language bias is a well-documented failure mode and augmentation has been explored, though not yet with LLM-based generation. Section~\ref{sec:chap3_rw_llm} then reviews the use of LLMs for paraphrasing and controlled text generation outside RSVQA, motivating the approach developed in this chapter.
...
\textcite{wei2019eda} proposed Easy Data Augmentation (EDA),
...
\textcite{edunov2018backtranslation} later studied this
...
than performing genuine visual reasoning, and data
augmentation has emerged as a primary strategy to expose
models to more diverse question distributions and reduce
this reliance.
\textcite{gokhale2020seada} introduce Semantic Equivalent
...
\textcite{sheng2021contrastclassify} propose a contrastive
...
paraphrases. \textcite{chen2022rethinkingDA} propose KDDAug,
...
across question types without human-designed rules. Across these approaches, robustness is obtained either by
generating harder paraphrases at training time or by
explicitly regularizing the model toward paraphrase
invariance; both directions assume a source of linguistic
variation rich enough to be informative, an assumption
that is far more difficult to satisfy in RS, where questions
are automatically generated from a narrow set of templates
rather than collected from human annotators.
 
In the remote sensing domain specifically, the
generalization problem is more severe than in standard
VQA. \textcite{chappuis2025langbias} provide a systematic
...
\textcite{yuan2023adversarial} address this from the model
...
\textcite{yuan2023multilingual}, who apply back translation
...
\textcite{meng2022lmcppf} demonstrate that few-shot
...
\textcite{dai2023llmaugsurvey} provide a comprehensive
...
output quality. \textcite{whitehouse2023llmcrosslingual}
...
\label{sec:chap3_positioning}
 
Within RS, \textcite{chappuis2025langbias} diagnose language bias but do not propose a data-centric solution, and \textcite{yuan2023adversarial} address it from the model side through adversarial regularization rather than by enriching the training distribution. The most directly comparable prior work, \textcite{yuan2023multilingual}, does enrich the training distribution, but through back translation, which Section~\ref{sec:chap3_rw_nlp} showed remains structurally close to the original questions and degrades on RS-specific vocabulary. This chapter replaces back translation with instruction-tuned LLM generation as the augmentation source, and evaluates directly whether the resulting syntactic diversity translates into measurable robustness gains on unseen RSVQA phrasings, rather than assuming it does.

\section{LLM-Based Question Reformulation for RSVQA}
...
question formulations. Figure~\ref{fig:pipeline} provides
an overview of the full pipeline, detailed in the
...
prohibitive). LLaMA-2 has fully open-weights and runs
locally.
...
versions of a given question. The full set of prompts and their associated keywords
is summarized in Table~\ref{tab:prompts}. The prompts were designed
...
question was manually classified as \textit{correct}
...
evaluation are presented in Table~\ref{tab:perf_prompts}.
...
\begin{table*}[!t]
  ...
  \footnotesize
   \resizebox{\textwidth}{!}{ % Adjust the width as needed
    ...
    \cline{2-11}
    &Qual (\%) & 38.3 & 48.3 & 1.7 & 26.7 & 30 & 55 & 58.3 & 63.3 & 35\\
    \hline

  \end{tabular}
  }
  \caption{Performances of different prompts (p1–p9) (see Table \ref{tab:prompts}) in augmenting questions across Easy, Medium, and Hard complexities. The numbers represent correct/incorrect/borderline responses for each question.}

  \label{tab:perf_prompts}
\end{table*}
...
the same meaning.''}) achieved the highest quality
...
% ============================================================
 This section evaluates whether the LLM-based augmentation pipeline introduced in 
 Section~\ref{sec:chap3_method} improves robustness to unseen question phrasings without degrading in-distribution accuracy, 
 and compares it directly against back translation, the closest prior approach. Section~\ref{sec:chap3_dataset} describes the RSVQA-HR benchmark used throughout. 
 Section~\ref{sec:chap3_setup} details the models, training protocol, and the three evaluation splits used to isolate in-distribution performance from generalization to each augmentation strategy's own phrasings. 
 Section~\ref{sec:chap3_results} then reports and analyzes the results, including a breakdown by question type and by cross-augmentation generalization.
...
training). RSVQA-HR's inclusion of test set 2 makes
it well-suited to additionally support geographic
generalization evaluation, going beyond the linguistic
variation studied in this chapter; we leave this
...
of \cite{chappuis2025langbias}, we use a dual-encoder
...
sets: the original unaugmented dataset (Original), our proposed LLM-augmented dataset (LLM-Aug), and the
back-translation-augmented dataset (BT-Aug) \cite{yuan2023multilingual}. All
...
Table~\ref{tab:chap3_results} presents the cross-evaluation
...
distribution. In contrast, the BT-Aug model performs notably worse on
the original test set (69.07\%), a drop of roughly 13
points compared to the Original model (82.17\%). This
indicates that back-translation augmentation, unlike
LLM-based augmentation, introduces enough noise into the
training distribution to measurably degrade
in-distribution performance, likely a consequence of
the lower-quality paraphrases produced by the
translation pipeline, as discussed in
Section~\ref{sec:chap3_rw_nlp}.
...
The \textit{area} category shows the largest performance gap.
...
analysis in Section~\ref{sec:chap3_difficulty}: \textit{area}
questions tend to involve complex spatial descriptions
that are hardest to paraphrase faithfully, meaning
the LLM-generated \textit{area} questions are the most
