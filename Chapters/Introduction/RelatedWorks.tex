...
RSVQA was introduced by \textcite{lobry2020rsvqa} to allow users to query EO imagery through natural language questions such as "Is there a flood in this image?" or "How many buildings are in this area?". Since then, a large number of datasets have extended the task along three main axes: the image modality considered, the diversity and coverage of the questions, and the overall data volume. Table~\ref{tab:rw-vqa-datasets} summarizes the main characteristics of existing RSVQA datasets.

\textbf{Modality of interest.} The image modality is a defining characteristic of a dataset, since it bounds the type of information the answers can be grounded in: optical imagery supports questions about roof color or crop type, multispectral imagery supports questions about vegetation health or water quality, and SAR supports questions about building height or flood extent. The large majority of existing datasets are optical-only, spanning several acquisition scales. UAV-based datasets such as FloodNet-v2~\cite{sarkar2023floodnet} (Figure~\ref{fig:rw-floodnet-example}) and RescueNet~\cite{sarkar2023rescuenet} offer very fine resolution (0.015m) but limited spatial extent, aimed at post-disaster damage assessment. Datasets such as RSVQA-LR~\cite{lobry2020rsvqa} trade resolution (10m, Sentinel-2) for broad, freely available coverage over the Netherlands. Aerial orthophoto datasets sit in between: RSVQA-HR~\cite{lobry2020rsvqa} (0.15m, USGS orthophotos over US cities, Figure~\ref{fig:rw-rsvqahr-example}) and CRSVQA~\cite{zhang2023crsvqa} (0.3m) support more localized, fine-grained questions than satellite data while remaining cheaper to acquire than UAV surveys. HRVQA~\cite{li2024hrvqa} and EarthVQA~\cite{wang2024earthvqa} sit at similarly high resolution (0.08m and 0.3m respectively), the latter specifically targeting relational reasoning over Chinese urban scenes (Nanjing, Changzhou, Wuhan). RSIVQA~\cite{zheng2021rsivqa} instead pools images from five heterogeneous existing datasets (UCM, AID, Sydney, DOTA, HRRSD) spanning 0.15-8m, prioritizing modality diversity over a single consistent acquisition source.
...
\centering
\includegraphics[width=\textwidth]{images/relatedworks/RSVQA-HR-Example.jpg}
\caption{RSVQA-HR}
\label{fig:rw-rsvqahr-example}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{images/relatedworks/Floodnet-vqa-Example.png}
\caption{FloodNet-v2}
\label{fig:rw-floodnet-example}
\end{subfigure}
\caption{Example images, questions, and their categories from two datasets spanning very different acquisition scales: (a) RSVQA-HR, aerial orthophotos over US cities~\cite{lobry2020rsvqa}; (b) FloodNet-v2, UAV imagery for post-disaster damage assessment~\cite{sarkar2023floodnet}.}
\label{fig:rw-rsvqahr-floodnet}
\end{figure}

\textbf{Question diversity and coverage.} An ideal dataset would represent the full space of questions an end-user might ask. Fully manual annotation, as used for general-purpose VQA on natural images, remains rare at scale in RS due to its cost: FloodNet-v2 (43 unique questions), CRSVQA (674 unique questions), and RSIEval~\cite{hu2023rsgpt} (936 samples, purpose-built for evaluation rather than training) are the notable exceptions, but their scale limits the diversity they can offer. Most datasets instead derive questions automatically by filling predefined templates from existing labels or external geographic sources: RSVQA-HR derives questions from OpenStreetMap, while RSIVQA derives them from five source datasets' existing class labels. This approach necessarily bounds the achievable question diversity to what the source data covers; for instance, since OpenStreetMap does not track moving objects, no questions about cars can be generated from this source. 
...
Four open challenges recur across the RSVQA literature. 

\textbf{Multi-modality}: while SAR and multispectral data are freely available and complementary to optical imagery, fusing them effectively remains harder than fusing multiple optical sources, since SAR-specific artifacts (speckle noise, geometric distortion) interact with the fusion mechanism in ways optical-only architectures do not anticipate~\cite{tosato2025b}. 

\textbf{Answer space}: the classification-versus-generation tradeoff discussed under answer prediction above remains unresolved, with generative models offering more natural, open-ended answers at the cost of harder evaluation (Section~\ref{sec:rw-vqa-metrics}) and, as noted in Section~\ref{sec:rw-foundation}, frequently under-reported performance on numerical questions specifically. 

\textbf{Interpretability and explainability}: most architectures remain black boxes with respect to which image regions or reasoning steps produced a given answer; Checkmate~\cite{tosato2025checkmate} addresses this directly by identifying and pointing to the specific image regions supporting each answer, building on the segmentation and attention-based interpretability mechanisms discussed under image processing and fusion above. 

\textbf{Trustworthiness}: as discussed above, a model can achieve high accuracy while relying substantially on language bias rather than the image; $RelAcc$ provides a way to measure this, but mitigating it, rather than merely measuring it, remains largely open, with adversarial training~\cite{yuan2023badversarial} as one of the few dedicated attempts so far.
...
Cross-modal retrieval in RS is most commonly framed as image-text retrieval: given a textual query, retrieve the matching image(s) from an archive, or conversely. RSICD~\cite{lu2017exploring} and RSITMD~\cite{yuan2022exploring} are the standard benchmarks for this setting, repurposing captioning-style annotations and evaluating with recall@K metrics, while PatternNet~\cite{zhou2018patternnet} instead targets image-to-image retrieval (CBIR) based on visual similarity of scene patterns. This unimodal-query setting is well established and is not the focus of this section, which instead centers on composed image retrieval (CIR), where the query combines an image and a text rather than a single modality.  Figure~\ref{fig:rw-retrieval-types} illustrates image-image, text-to-image, and text-to-time-series retrieval as points of comparison before turning to CIR specifically.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\textwidth]{images/relatedworks/retrieval.png}
\caption{Three retrieval settings illustrated on the same underlying archive. Type 1 (CBIR) retrieves images from an image query. Type 2 (text-to-image) retrieves images from a text query. Type 3 (text-to-time-series) retrieves an image time series from a text query. A fourth setting, composed image retrieval (CIR), combines a reference image with modification text into the query; an example specific to RS is shown in Figure~\ref{fig:rw-patterncom-attrs}.}
\label{fig:rw-retrieval-types}
\end{figure}
...
\caption{The eight attribute use cases covered by PATTERNCOM: color, context, density, existence, quantity, shape, size, and texture. Each query pairs a reference image with an attribute-modifying text and must retrieve an image of the same class exhibiting the target attribute value; subfigures (b) and (d) extend this to queries spanning multiple classes and attributes. Figure from \cite{psomas2024composed}.}
...
Composed image retrieval (CIR) was introduced to RS by \textcite{psomas2024composed} together with the PATTERNCOM benchmark above. Their method, WEICOM, is training-free: it fuses image-to-image and text-to-image similarities from a frozen VLM (CLIP or RemoteCLIP, discussed further in Section~\ref{sec:rw-foundation}) via a modality control parameter that controls the relative weight given to the image and text parts of the query, avoiding the need for supervised composed-retrieval triplets that are costly to annotate at scale. This training-free design directly addresses the annotation bottleneck that motivated CoVR in the general vision-language literature, though via a different mechanism: WEICOM sidesteps the need for triplets altogether by fusing similarities at inference time, whereas CoVR sidesteps manual annotation by mining triplets automatically and still trains a dedicated composition-module-based model. Follow-up work benchmarks composed retrieval specifically for applied EO use cases~\cite{efthymiadis2026benchmarking}, further establishing CIR as a distinct evaluation setting in RS beyond this initial benchmark.

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{images/relatedworks/tirg_architecture.jpg}
...
These two families illustrate the two ways captioning and VQA have been coupled in the literature: captioning (or a captioning-like description) as an intermediate representation feeding VQA reasoning (Prompt-RSVQA, MT-prompt-RSVQA, PAN-RSVQA), and captioning and VQA as jointly supervised, parallel outputs of a shared model (RS-LLaVA, the UAV video model). Larger, task-agnostic VLMs that also cover captioning and VQA among several other capabilities (e.g. RSGPT, GeoChat) are discussed in Section~\ref{sec:rw-foundation}.
...
\textbf{Contrastive vision-language encoders.} A separate, encoder-only line of work adapts CLIP-style contrastive pretraining to RS imagery, producing a general-purpose visual-textual embedding space rather than a generative decoder. RemoteCLIP~\cite{liu2024remoteclip} is the reference model in this family and is used directly as the frozen encoder backbone in WEICOM (Section~\ref{sec:rw-retrieval-methods}). RS5M and GeoRSCLIP~\cite{zhang2024rs5m} scale this pretraining to a larger RS image-text corpus, and more recent encoders such as DGTRS-CLIP~\cite{chen2025dgtrsddgtrsclipdualgranularity}, which targets dual-granularity alignment, continue this line of work. All of these report unimodal image-text retrieval performance as a core evaluation of alignment quality, and any of them could in principle serve as the frozen encoder backbone for a WEICOM-style composed retrieval method, though to date only CLIP and RemoteCLIP have been evaluated in that specific composed setting. Because these encoders are trained without any task-specific supervision, they are equally applicable to retrieval, zero-shot classification, and as a visual encoder for VQA or captioning models.

\textbf{Self-supervised vision-only foundation models.} This family drops language entirely, pretraining a vision-only encoder on unlabeled RS imagery through masked image modeling or contrastive objectives, then adapting it to downstream tasks via linear probing or fine-tuning. SatMAE~\cite{cong2022satmae} adapts masked autoencoding to satellite imagery with dedicated spectral and temporal variants, addressing the fact that RS imagery has more spectral bands and temporal structure than the natural images MAE was originally designed for. Scale-MAE~\cite{reed2023scalemae} instead targets the resolution variability across RS sensors, using a band-pass filter to reconstruct low- and high-frequency image content at different scales, so that a single pretrained encoder transfers across sensors with very different ground sampling distances. CROMA~\cite{fuller2023croma} combines contrastive and masked-autoencoding objectives specifically across paired SAR and optical imagery, addressing multi-sensor alignment through self-supervision rather than through a shared embedding space trained with text as in the contrastive encoders above. DOFA~\cite{xiong2024dofa} generalizes further still, using a wavelength-conditioned dynamic patch embedding so that a single model adapts its input processing to whichever sensor modality it receives, rather than being retrained or re-architected per sensor. Prithvi~\cite{jakubik2023prithvi}, developed by NASA and IBM, targets multi-temporal Harmonized Landsat-Sentinel data specifically, with 3D patch and positional embeddings to capture spatiotemporal structure. AnySat~\cite{astruc2024anysat} pushes the multi-sensor generalization seen in DOFA further still, using a resolution-adaptive joint embedding so that a single self-supervised model unifies resolutions, scales, and modalities within one Earth observation model rather than being specialized to a fixed sensor configuration.  RAMEN~\cite{ramen} takes this a step further by treating spatial resolution as a controllable output parameter rather than a fixed property of the input: a single masked-reconstruction transformer, trained across diverse EO sources and sensor configurations, lets the user choose the desired level of spatial detail at inference time, trading off precision against computational cost, and transfers to both seen and unseen sensor configurations on the PANGAEA benchmark. None of these models are evaluated on RSVQA, captioning, or retrieval directly; their relevance to this thesis is as potential frozen visual backbones, in the same role RemoteCLIP already plays in WEICOM, rather than as end-to-end systems for the three tasks this chapter reviews.

\textbf{Predictive (JEPA-based) architectures.} A more recent self-supervised paradigm, the joint-embedding predictive architecture (JEPA)~\cite{assran2023jepa}, predicts masked content in representation space rather than reconstructing pixels, avoiding the augmentation- and decoder-dependent design of contrastive and masked-autoencoding methods respectively. Unlike the vision-only foundation models above, several JEPA-based methods in RS target retrieval directly. SAR-JEPA~\cite{li2023sarjepa} applies the paradigm to SAR automatic target recognition, predicting gradient-domain representations rather than raw pixels to cope with speckle noise. ReJEPA~\cite{choudhury2025rejepa} targets RS image retrieval specifically, learning representations across SAR, multispectral, and RGB imagery for content-based retrieval.

Table~\ref{tab:rw-foundation-models} summarizes the models discussed in this section across all four families, alongside the modalities and task orientation of each.
...
\hline
\textbf{Model} & \textbf{Family} & \textbf{Modality / sensors} & \textbf{Task orientation} \\
\hline
RSGPT~\cite{hu2023rsgpt} & Instruction-tuned VLM & Optical & Captioning, VQA \\
GeoChat~\cite{kuckreja2024geochat} & Instruction-tuned VLM & Optical & VQA, region grounding \\
LHRS-Bot~\cite{muhtar2025lhrsbot} & Instruction-tuned VLM & Optical & VQA, grounding \\
SkyEyeGPT~\cite{zhan2025skyeyegpt} & Instruction-tuned VLM & Optical & Multiple RS VL tasks \\
EarthGPT~\cite{zhang2024earthgpt} & Instruction-tuned VLM & Optical, SAR, infrared & Multiple RS VL tasks \\
SkySenseGPT~\cite{luo2024skysensegptfinegrainedinstructiontuning} & Instruction-tuned VLM & Multi-sensor & Fine-grained VL understanding \\
RSUniVLM~\cite{liu2024rsunivlmunifiedvisionlanguage} & Instruction-tuned VLM & Multi-sensor & Unified multi-granularity VL \\
GeoRSMLLM~\cite{zhang2025georsmllmmultimodallargelanguage} & Instruction-tuned VLM & Multi-sensor & Unified multi-granularity VL \\
TEOChat~\cite{irvin2025teochatlargevisionlanguageassistant} & Instruction-tuned VLM & Optical, temporal & Temporal VQA \\
UniRS~\cite{li2024unirsunifyingmultitemporalremote} & Instruction-tuned VLM & Optical, temporal & Temporal VQA \\
\hline
RemoteCLIP~\cite{liu2024remoteclip} & Contrastive VL encoder & Optical & Retrieval, zero-shot classification \\
GeoRSCLIP~\cite{zhang2024rs5m} & Contrastive VL encoder & Optical & Retrieval, zero-shot classification \\
DGTRS-CLIP~\cite{chen2025dgtrsddgtrsclipdualgranularity} & Contrastive VL encoder & Optical & Dual-granularity retrieval \\
\hline
SatMAE~\cite{cong2022satmae} & Self-supervised vision-only & Multispectral, temporal & Classification, segmentation \\
Scale-MAE~\cite{reed2023scalemae} & Self-supervised vision-only & Optical, multi-resolution & Classification, segmentation \\
CROMA~\cite{fuller2023croma} & Self-supervised vision-only & SAR + optical & Classification, segmentation \\
DOFA~\cite{xiong2024dofa} & Self-supervised vision-only & Multi-sensor (dynamic) & Classification, segmentation, detection \\
Prithvi~\cite{jakubik2023prithvi} & Self-supervised vision-only & Optical, temporal & Segmentation, land cover, flood mapping \\
AnySat~\cite{astruc2024anysat} & Predictive / joint-embedding & Multi-resolution, multi-sensor & General EO representation \\
RAMEN~\cite{ramen} & Self-supervised vision-only & Multi-resolution, multi-sensor & General EO representation, adjustable output resolution \\
\hline
SAR-JEPA~\cite{li2023sarjepa} & Predictive (JEPA) & SAR & Target recognition \\
ReJEPA~\cite{choudhury2025rejepa} & Predictive (JEPA) & SAR, multispectral, RGB & Image retrieval \\
\hline
\end{tabularx}
\caption{Task-agnostic RS foundation models discussed in this chapter, organized by family. Task orientation reflects what each model is evaluated on in its source paper, not what it could in principle be adapted to.}
\label{tab:rw-foundation-models}
\end{table}
